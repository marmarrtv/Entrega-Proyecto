  ...
  // Inicializar autocompletado para profesores
  autocompleteProfesores(profesorInput);


  function cargarHerramientas() {
    const inventario = JSON.parse(localStorage.getItem("inventario")) || [];
    const herramientas = inventario.filter(item => item.tipo === "Herramienta" && parseInt(item.cantidad) > 0);


    herramientaSelect.innerHTML = '<option value="" disabled selected>Selecciona una herramienta</option>';
    herramientas.forEach(h => {
      const option = document.createElement("option");
      option.value = h.nombre;
      option.textContent = `${h.nombre} (${h.cantidad} disponibles)`;
  ...
  function guardarPedido(pedido) {
    ...
    pedido.tipo = "Herramienta";
   
    // Agregar hora exacta
    const ahora = new Date();
    pedido.hora = ahora.toLocaleTimeString();
   
    // Guardar en pedidos de herramientas
    pedidosHerramientas.push(pedido);
    localStorage.setItem("pedidosHerramientas", JSON.stringify(pedidosHerramientas));
  ...
  document.getElementById("pedidoForm").addEventListener("submit", function (e) {
    ...
    const nombreHerramienta = herramientaSelect.value;
